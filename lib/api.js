...
var Api = function Api(provider) {
    ...
    var send = function send(method) {
        return function () {
            ...
            return new Promise(function (resolve, reject) {
                provider.send(method, params).then((result, err) => {
    ...
    var encodeABI = function encodeABI(type, value) {
        if (type === "bytes") {
            ...
            return { data: Bytes.fromUint8(value), dynamic: false }
        } else {
    ...
    var callWithDefaults = async function callWithDefaults(tx, block) {
